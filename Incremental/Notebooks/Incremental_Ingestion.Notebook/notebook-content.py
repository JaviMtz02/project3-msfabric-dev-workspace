...
josh_key = '06909e5e8bed2721d821eab92a26440f19826a2e2e00d3df8a5ce0ad8a5a2cdf'
collin_key = '7f633d8879ca8c19b8a7381094e5998f5e33d6adff5751929d4e588a34c1c0d5'
...
for id in country_ids_list:
    ...
    for row in sensor_ids.toLocalIterator():
        ...
        while True:
            ...
            try:
                resp = openaq_conn.measurements.list(sensors_id=sid, data='days', datetime_from=datetime_from_str, datetime_to=datetime_to_str, limit=1000)
                break
            except TimeoutError:
                print(row)
...
location_df = spark.read.format('delta').load(f'Files/Bronze/Locations/country_id=10')
...
target_row = (
    df_rn
    .filter(df_rn.sensor_id == 1839417)
    .select("row_num")
    .limit(1)
    .collect()[0][0]
)
...
for row in after_df.toLocalIterator():
    ...
    if dtt > datetime.now(timezone.utc):
        while dtt > datetime.now(timezone.utc) and dtt > 0:
...
openaq_conn.measurements.get(sensor_ids= )
